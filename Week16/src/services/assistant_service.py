"""
Legacy Single-Pass Assistant Orchestrator for ShopAssist AI (Week 15 Backwards Compatibility).
...
class AssistantOrchestrator:
    ...
    def process_query(self, req: AssistantRequest) -> AssistantResponsePayload:
        ...
        # 4. Tool Calling (single-pass regex heuristic)
        ...
        user_prompt = f"User Query: {req.query}\nRAG Context: {rag_context}\nTool Result: {tool_exec_result}"
        system_prompt = "You are ShopAssist AI Customer Support Assistant. Return valid JSON."
        ...
        structured = llm_result.structured_response or AssistantStructuredResponse(
            intent=onnx_pred.intent,
            confidence=0.9,
            thought_process="Single-pass pipeline resolution",
            final_response="Thank you for contacting ShopAssist AI."
        )
